...
export function CustomMonthlyDay<DayData>({
  renderDay,
  onDateClick,
  className,
}: MonthlyDayProps<DayData>) {
    ...
    <button
      ...
      className={cn(
        ...
        requests.some((r: { date: string; status: string; type: string }) =>
          ...
          ? requests.find((r: { date: string; status: string; type: string }) =>
              isSameDay(r.date, day)
            )?.status === "pending"
      ...
      <PopoverContent className="w-36">
        <ToggleGroup
          ...
          onValueChange={(value) => {
            onDateClick((prev?: any[]) => {
              ...
              const currentDayEvents = currentEvents.filter((e) =>
                isSameDay(e.date, day)
              );
              const otherDayEvents = currentEvents.filter(
                (e) => !isSameDay(e.date, day)
              );

              const typesToAdd = value.filter(
                (type) => !currentDayEvents.some((e) => e.type === type)
              );
              const typesToKeep = currentDayEvents.filter((e) =>
                value.includes(e.type)
              );
        ...
        >
          {(() => {
            ...
            const availableToggleTypes = toggleTypes.filter(
              (toggleType) =>
                !requests.some(
                  (r: { date: string; type: string }) =>
...
export const ApplyMonthlyEventItem = ({ type }: DefaultEventItemProps) => {
  return (
    <li>
